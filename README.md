...
The catalog search is a keyboard-accessible typeahead. The catalog document requires an external stylesheet and JavaScript module through `cx.require_asset`, then sends the product index under an autogenerated key with `cx.send_json`. In streaming mode the browser starts loading those assets before the deferred catalog panels finish, and the module awaits the separately streamed JSON through `topcoat.json(key)`.
